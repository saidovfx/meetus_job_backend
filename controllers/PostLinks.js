...
module.exports.deleteContact = async (req, res) => {
    try {
        ...
        if (!deleteObject) return res.status(400).json({ message: "Delete object is NaN" });
        ...
        if (!validKeys.includes(deleteObject))
            return res.status(400).json({ message: `Message ${deleteObject} is not equal to any contact` });

       user.socialLinks[deleteObject]=''
        user.markModified("socialLinks");
        await user.save();

        res.status(200).json({ message: `${deleteObject} contact successfully deleted`, user });
    } catch (error) {
        res.status(500).json({ message: "Server error", error: error.message });
    }
};
