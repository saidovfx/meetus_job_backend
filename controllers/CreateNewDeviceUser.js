...
module.exports.createUniqueUser = async (req, res) => {
  try {
    ...
    if (existingUniqueUser)
      return res.status(409).json({ message: "Unique user already exists" });
...
module.exports.interNewUser = async (req, res) => {
  try {
    ...
    if (!uniqueId)
      return res.status(401).json({ message: "Unique user Id is undefined" });
    if (!userId)
    ...
    if (!uniqueUser)
      return res.status(404).json({ message: "Unique user not found" });
    const userIDFilter = uniqueUser.belowAccounts.find(
      (i) => i.userId === userId
    );
    if (userIDFilter) {
      uniqueUser.belowAccounts = uniqueUser.belowAccounts.filter(
        (i) => i.userId !== userId
      );
      ...
      return res
        .status(200)
        .json({ message: "User account updated", uniqueUser });
    }
